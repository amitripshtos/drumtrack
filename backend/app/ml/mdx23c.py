"""MDX23C TFC-TDF-net model architecture.
...
class TFC_TDF_net(nn.Module):
    def __init__(self, config):
        ...
        norm = _get_norm(norm_type=config.model.norm)
        act = _get_act(act_type=config.model.act)

        # Determine number of target instruments
        if getattr(config.training, "target_instrument", None):
            self.num_target_instruments = 1
        else:
            self.num_target_instruments = len(config.training.instruments)
