...
export function DawPlayerDialog() {
  ...
  const handleTimelineScroll = useCallback((e: React.UIEvent<HTMLDivElement>) => {
    if (isSyncing.current) return;
    isSyncing.current = true;
    if (channelScrollRef.current) channelScrollRef.current.scrollTop = e.currentTarget.scrollTop;
    requestAnimationFrame(() => {
      isSyncing.current = false;
    });
  }, []);
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed inset-0 z-50 flex flex-col bg-background text-foreground">
