...
export function DawTimeline({ channels, scrollRef, onVerticalScroll }: DawTimelineProps) {
  const { currentTime, duration, engine, isPlaying, events } = usePlayerContext();
  ...
  useEffect(() => {
    ...
    setWaveformPeaks(computePeaks(buffer, numBins));
  ...
  useEffect(() => {
    ...
    if (playheadX > viewEnd - margin) setScrollLeft(playheadX - containerWidth * 0.3);
  ...
  const handleWheel = useCallback((e: React.WheelEvent) => {
    if (e.ctrlKey || e.metaKey) {
      ...
      setPixelsPerSecond((prev) => Math.min(500, Math.max(20, prev * factor)));
    }
  }, []);
    ...
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden" onWheel={handleWheel}>
      ...
      >
        <div style={{ width: totalWidth, position: "relative" }}>
          ...
          {channels.map((ch) => (
            <div key={ch.id} className="border-b border-border/50" style={{ height: LANE_HEIGHT }}>
              <DawTrackLane
                channel={ch}
                events={events}
                duration={duration}
